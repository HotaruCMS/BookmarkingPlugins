...
class AkismetFunctions {
	 ...
	 */
	public function __construct($h, $blogURL, $wordPressAPIKey) {
		$this->blogURL = $blogURL;
		$this->wordPressAPIKey = $wordPressAPIKey;
                $this->serverKeys = $h->cage->server;
		...
		$this->comment['blog'] = $blogURL;
                ...
                $user_agent = $h->cage->server['HTTP_USER_AGENT'];
                $referer = $h->cage->server['HTTP_REFERER'];
                $remote_addr = $h->cage->server['REMOTE_ADDR'];
		 ...
		 * PHP5 one...
		 */
		if(isset($remote_addr) && $remote_addr != getenv('SERVER_ADDR')) {
			$this->comment['user_ip'] = $remote_addr;
		} else {
	...
	private function getQueryString() {                
		foreach($this->serverKeys as $key => $value) {
