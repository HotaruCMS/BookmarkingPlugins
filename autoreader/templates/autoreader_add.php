...
<?php 
	...
	if ($action=='edit') {
		...
		<ul id="edit_buttons" class="submit">
			<?php if ($h->vars['autoreader']['wpo_help']) { ?>
		...
		<div class="section" id="section_options">
			...
			<?php if ($h->vars['autoreader_settings']['wpo_premium']) { ?>
				<div class="checkbox">
					...
					<?php  echo checkbox_tag('campaign_cacheimages', 1, _data_value($data['main'], 'cacheimages', is_writable($h->vars['autoreader_settings']['wpo_cachepath']))) ?>
					<p class="note">Images will be stored in your server, instead of hotlinking from the original site.
							<?php if ($h->vars['autoreader']['wpo_help']) { ?><a href="helpurl image_caching" class="help_link">More</a><?php } ?></p>
				</div>
			<?php } ?>
	...
	<div class="text required">
				<?php echo label_for('campaign_trunc', 'Number of characters to truncate at for each post') ?>
				<?php echo input_tag('campaign_trunc', _data_value($data['main'], 'trunc', '200'), 'size=2 maxlength=4') ?>
				<p class="note">The greater the amout of characters the more your database will fill up quickly.</p>
			</div>
...
<script type='text/javascript'>
	...
	$("#tool_removeall").click(function(){
		...
		if (answer) {
				...
				var sendurl = BASEURL + 'admin_index.php?page=plugin_settings&plugin=autoreader&template=autoreader_add';
