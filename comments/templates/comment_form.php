...
<?php } // JavaScript changes this form! See comments.js ?>
    ...
    <form class='form' name='comment_form' action='<?php echo $h->url(array('page' => $h->comment->postId)); ?>' method='post' onsubmit="document.getElementById('comment_submit_<?php echo $h->comment->id; ?>').disabled = true; return true;">
        ...
        <input type="submit" name="submit" id="comment_submit_<?php echo $h->comment->id; ?>" value="<?php echo $h->lang['comments_form_submit']; ?>" class="submit btn" />
        ...
        <input type="hidden" name="comment_post_id" value="<?php echo $h->comment->postId; ?>" />
        ...
        <?php if (($h->comment->id != 0) && ($h->currentUser->getPermission('can_set_comments_pending') == 'yes')) { ?>
            <div class='comment_status' style='display: none;'>
                <a href="<?php echo BASEURL; ?>index.php?page=comments&amp;action=setpending&amp;cid=<?php echo $h->comment->id; ?>&amp;pid=<?php echo $h->comment->postId; ?>">
                    <?php echo $h->lang['comments_form_set_pending']; ?>
                </a><br />
                
                <?php if ($h->currentUser->getPermission('can_delete_comments') == 'yes') { ?>
                <a class="bold_red" href="<?php echo BASEURL; ?>index.php?page=comments&amp;action=delete&amp;cid=<?php echo $h->comment->id; ?>&amp;pid=<?php echo $h->comment->postId; ?>">
